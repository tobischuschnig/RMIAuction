...
public class TaskExecuter {
	...
	public TaskExecuter(ManagementClient c, String billingServer,
		...
		try {
			managementClientInterface = (ManagementClientInterface) UnicastRemoteObject
					.exportObject(c, 0);
	...
	public boolean login(String username, String password) {
		boolean ret = false;
		try {
			secure = (BillingServerSecureInterface) objb.login(username,
					password);
			if (secure != null) {
				ret = true;
			}
	 ...
	 */
	public boolean addStep(double startPrice, double endPrice,
			double fixedPrice, double variablePricePercent) {
		boolean ret = false;

		try {
			ret = secure.createPriceStep(startPrice, endPrice, fixedPrice,
					fixedPrice);
	...
	public void bill(String username) {
		try {
			ArrayList<Bill> bills = secure.getBill(username);
			if (bills.size() == 0) {
				cli.outln("No Bills for User " + username);
			} else
				for (int x = 0; x < bills.size(); x++) {
					cli.outln(bills.get(x).toString());
				}
	...
	public void subscribe(String filter) {
		try {
			...
			cli.outln("Server said: "
					+ obja.suscribe(filter, managementClientInterface));
	...
	public void unsubscribe(String subscriptionID) {
		...
		try {
			...
			cli.outln("Failed unsubscribe " + subscriptionID + ": "
					+ ex.getMessage());
		}
